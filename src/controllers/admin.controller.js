...
const getServiceModel = (type) => {
  switch (type) {
    case 'hotel': case 'hotels': return Hotel;
    case 'flight': case 'flights': return Flight;
    case 'train': case 'trains': return Train;
    case 'bus': case 'buses': return Bus;
    case 'taxi': case 'taxis': return Taxi;
    case 'restaurant': case 'restaurants': return Restaurant;
    case 'guide': case 'guides': return Guide;
    default: return null;
  }
};
...
export const getDashboardStats = async (req, res) => {
  try {
    const [
      hotelCount, flightCount, trainCount, busCount, taxiCount, restaurantCount, guideCount,
      bookingStats, userCount, recentBookings
    ] = await Promise.all([
    ...
    res.status(200).json({
      ...
      data: {
        services: {
          ...
          guides: guideCount,
          total: hotelCount + flightCount + trainCount + busCount + taxiCount + restaurantCount + guideCount
        },
        bookings: bookingStats[0] || { totalBookings: 0, totalRevenue: 0, confirmedBookings: 0, pendingBookings: 0, cancelledBookings: 0 },
        users: userCount,
        recentBookings
      }
    });
...
export const updateService = async (req, res) => {
  try {
    ...
    const service = await Model.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
...
export const updateUser = async (req, res) => {
  try {
    const { id } = req.params;
    const { role, subscriptionTier, subscriptionStatus, standardCredits } = req.body;
